...
class UnmergeSubCommand extends SubCommand
{
	...
	public function execute(CommandSender $sender, array $args) : bool {
		...
		if(!$this->getPlugin()->isLevelLoaded($levelName)) {
			$sender->sendMessage(TextFormat::RED . $this->translateString("unmerge.notinplotworld"));
			return true;
		}
		...
		if(count($plotIdArray) != 2 or !is_numeric($plotIdArray[0]) or !is_numeric($plotIdArray[1])) {
			$sender->sendMessage(TextFormat::RED . $this->translateString("unmerge.wrongid"));
			return true;
		}
		$plot = $this->getPlugin()->getProvider()->getPlot($levelName, (int) $plotIdArray[0], (int) $plotIdArray[1]);
		if($plot->owner == "" and !$sender->hasPermission("myplot.admin.unmerge")) {
			$sender->sendMessage(TextFormat::RED . $this->translateString("unmerge.unclaimed"));
			return true;
		}
		/** @var string[] $plotIdArray */
		...
		if($plot2->owner == "" and !$sender->hasPermission("myplot.admin.unmerge")) {
			$sender->sendMessage(TextFormat::RED . $this->translateString("unmerge.unclaimed"));
			return true;
		}
		if($this->getPlugin()->unMergePlots($plot, $plot2)) {
			$plot = TextFormat::GREEN . $plot . TextFormat::WHITE;
			$plot2 = TextFormat::GREEN . $plot2 . TextFormat::WHITE;
			$sender->sendMessage($this->translateString("unmerge.success", [$plot2, $plot]));
		}else{
