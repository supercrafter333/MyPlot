...
class BorderCorrectionTask extends Task {
	...
	public function __construct(MyPlot $plugin, Plot $start, Plot $end, bool $fillCorner = false, int $cornerDirection = -1, int $maxBlocksPerTick = 256) {
		...
		$this->mergeBB = new AxisAlignedBB(
			min($this->plotBeginPos->x, $this->plotBeginPos->x + $plotSize + $roadWidth),
			0,
			min($this->plotBeginPos->z, $this->plotBeginPos->z + $plotSize + $roadWidth),
			max($this->plotBeginPos->x, $this->plotBeginPos->x + $plotSize + $roadWidth),
			$this->plotBeginPos->getLevel()->getWorldHeight(),
			max($this->plotBeginPos->z, $this->plotBeginPos->z + $plotSize + $roadWidth)
		);
	...
	public function onRun(int $currentTick) : void {
		...
		if($this->fillCorner) $this->plugin->getScheduler()->scheduleDelayedTask(new CornerCorrectionTask($this->plugin, $this->plot, $this->endPlot, $this->cornerDirection), 10);
